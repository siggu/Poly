...
def render_chatbot_main():
    ...
    # 입력창 - 로딩 중일 때는 폼 대신 안내 메시지 표시
    if is_loading:
        st.markdown(
            """
            <div style="padding: 12px; background-color: #f0f0f0; border-radius: 8px; text-align: center; color: #666;">
    ...
    # --- 대화 저장 및 초기화 UI (로딩 중이 아닐 때만 표시) ---
    if not is_loading:
        st.markdown("---")
    if not is_loading and st.session_state.save_chat_confirmation:
        st.warning(
