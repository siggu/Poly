-- ========================================
-- 임베딩 차원 변경: 768 → 1536
-- 모델: jhgan/ko-sroberta-multitask → OpenAI text-embedding-3-small
-- ========================================

-- 1. 기존 embeddings 테이블 백업
CREATE TABLE IF NOT EXISTS embeddings_backup_768 AS
SELECT * FROM embeddings;

-- 백업 확인
SELECT COUNT(*) as backup_count FROM embeddings_backup_768;

-- 2. 기존 embeddings 테이블 삭제
DROP TABLE IF EXISTS embeddings CASCADE;

-- 3. 새 차원(1536)으로 embeddings 테이블 재생성
CREATE TABLE embeddings (
    id SERIAL PRIMARY KEY,
    doc_id INT NOT NULL,
    field VARCHAR(50) NOT NULL,
    embedding vector(1536),  -- 768 → 1536 변경 (OpenAI text-embedding-3-small)
    UNIQUE(doc_id, field)
);
...
-- 완료 메시지
SELECT '✅ 스키마 변경 완료! 이제 OpenAI API로 임베딩을 재생성해야 합니다.' as status;

-- ========================================
-- 롤백 방법 (문제 발생 시):
-- ========================================
-- DROP TABLE IF EXISTS embeddings CASCADE;
-- ALTER TABLE embeddings_backup_768 RENAME TO embeddings;
